...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # setting flash to false, to run slow attention
        self.flash = False
...
class Block(nn.Module):
    ...
    def forward(self, x, past_key_values=None):
        attn_output, present_kv = self.attn(self.ln_1(x), past_key_values)
        x = x + attn_output
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, past_key_values=None):
        ...
        pos = torch.arange(pos_start, pos_start + t, dtype=torch.long, device=device) # shape (t)
        ...
        for i, block in enumerate(self.transformer.h):
            layer_cache_old = past_key_values[i] if past_key_values is not None else None
            x, layer_cache_new = block(x, layer_cache_old)
            new_kv_caches.append(layer_cache_new)
        ...
        return logits, loss, tuple(new_kv_caches)
